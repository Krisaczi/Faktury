...
import { Loader as Loader2, FileText, Send, CircleCheck as CheckCircle, TriangleAlert as AlertTriangle, Eye, Save, Ban, Calendar, Receipt, Info } from 'lucide-react';
...
export function PlatformInvoiceModal({
  ...
  function computeTotals() {
    ...
    for (const li of lineItems) {
      ...
      subtotal += amount;
      if (li.taxable) tax += Math.round(amount * TAX_RATE);
    }
  ...
  function updateLineItem(idx: number, field: keyof LineItem, value: string | number | boolean) {
    setLineItems((prev) => prev.map((li, i) => i === idx ? { ...li, [field]: value } : li));
  }

  function addLineItem() {
    setLineItems((prev) => [...prev, { description: '', quantity: 1, unitPriceCents: 0, taxable: true }]);
  }
          ...
          <div className="space-y-5">
            ...
            <div className="rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 p-4 space-y-3">
              ...
              <div className="grid grid-cols-5 gap-2 text-center">
                <div>
                  ...
                  <p className="text-sm font-medium text-slate-700 dark:text-slate-300">{usage.plan.name}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-400">Użytkownicy</p>
                  <p className="text-sm font-medium text-slate-700 dark:text-slate-300">{usage.usage.activeUsers}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-400">Dostawcy</p>
                  <p className="text-sm font-medium text-slate-700 dark:text-slate-300">{usage.usage.vendorCount}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-400">Faktury</p>
                  <p className="text-sm font-medium text-slate-700 dark:text-slate-300">{usage.usage.invoiceCount}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-400">Wystawione</p>
                  <p className="text-sm font-medium text-slate-700 dark:text-slate-300">{usage.usage.issuedInvoiceCount}</p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              {lineItems.map((li, idx) => (
                <div key={idx} className="grid grid-cols-12 gap-2 items-start p-2 rounded-lg border border-slate-200 dark:border-slate-700">
                  <Input
                  ...
                  <Input
                    ...
                    placeholder="Cena (gr)"
                  />
                  <div className="col-span-2 text-sm flex items-center h-8">
                    {formatCents(Math.round(li.quantity * li.unitPriceCents))}
                  </div>
                  <Button
                    variant="ghost" size="sm"
                    className="col-span-1 h-8 p-0 text-red-400 hover:text-red-600"
            ...
            <div className="rounded-lg border border-slate-200 dark:border-slate-700 p-4 space-y-1.5">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium text-slate-700 dark:text-slate-300">{formatCents(totals.subtotal)}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-500">VAT (23%)</span>
                <span className="font-medium text-slate-700 dark:text-slate-300">{formatCents(totals.tax)}</span>
              </div>
              <div className="flex justify-between text-base font-bold border-t border-slate-200 dark:border-slate-700 pt-1.5">
                ...
                <span className="text-blue-600 dark:text-blue-400">{formatCents(totals.total)}</span>
              </div>
            </div>
            ...
            <div className="rounded-lg border border-slate-200 dark:border-slate-700 p-6 space-y-4">
              ...
              <div className="flex justify-end space-y-1 flex-col items-end">
                <div className="flex justify-between w-48 text-sm">
                  ...
                  <span className="text-slate-700 dark:text-slate-300">{formatCents(totals.subtotal)}</span>
                </div>
                <div className="flex justify-between w-48 text-sm">
                  <span className="text-slate-500">VAT 23%:</span>
                  <span className="text-slate-700 dark:text-slate-300">{formatCents(totals.tax)}</span>
                </div>
        ...
        <DialogFooter className="gap-2">
          ...
          {step === 'preview' && (
            <>
              ...
              <Button size="sm" onClick={issueInvoice} disabled={isPending} className="gap-2 bg-blue-600 hover:bg-blue-700 text-white">
