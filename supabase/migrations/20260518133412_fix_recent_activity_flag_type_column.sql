/*
  # Fix get_homepage_stats RPC

  The previous version used `status = 'completed'` which is not a valid value
  in the report_status enum. Valid values are: draft, published, archived.
  Changed to `status = 'published'` to select completed/visible reports.
*/
